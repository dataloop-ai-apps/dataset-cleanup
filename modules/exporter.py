...
class Exporter:
    ...
    def install_start_exec(self, dpk_name, service_name, funtion_name, exec_type):
        ...
        execution = project.executions.create(function_name=funtion_name, service_id=service.id, execution_input={'dataset': {'dataset_id': self.dataset.id}, 'query': None})
        thread = threading.Thread(target=self.wait_for_execution, kwargs={"execution_id": execution.id, "exec_type": exec_type})
        thread.daemon = True
        thread.start()
    ...
    def start_execution(self, exec_type):
        ...
        if exec_type == 'clip':
            ...
            self.install_start_exec('quality-score-generator', 'quality-scores-generator', 'dataset_scores_generator', 'quality-score-generator')
